...
const fetchRawMaterials = async (): Promise<RawMaterial[]> => {
  ...
  if (!res.ok) throw new Error("Échec de la récupération des matières premières");
  return res.json();
};
...
export default function RawMaterialsPage() {
  ...
  const handleAdd = () => {
    router.push("/raw-materials/add");
  };

  const handleEdit = (material: RawMaterial) => {
    router.push(`/raw-materials/edit/${material.id}`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}raw-materials/${selectedMaterial.id}`,
        { method: "DELETE" },
      );
      if (!res.ok) throw new Error("Échec de la suppression");

      await refetch();
      showToast(`✅ Matière première ${selectedMaterial.name} supprimée`);
    } catch (err) {
      showToast("❌ Échec de la suppression de la matière première");
    } finally {
      setDialogOpen(false);
  ...
  const columns: MRT_ColumnDef<RawMaterial>[] = [
    {
    ...
    <Toast.Provider swipeDirection="right">
      <div className="p-6 min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
        ...
        {isError && (
          ...
          >
            <Toast.Title>❌ Échec de la récupération des matières premières</Toast.Title>
          </Toast.Root>
        )}
