...
const fetchDriverById = async (id: string): Promise<Driver> => {
  ...
  if (!response.ok) throw new Error("Erreur lors du chargement du chauffeur");
  return response.json();
};
...
const updateDriver = async ({ id, data }: { id: number; data: Partial<Driver> }): Promise<Driver> => {
  ...
  if (!response.ok) {
    ...
    throw new Error(error.message || "Erreur lors de la mise à jour du chauffeur");
  }
  return response.json();
};
...
function ModifierDriverContent({ id }: { id: string }) {
  ...
  useEffect(() => {
    fetchDriverById(id)
      ...
      .catch((error) => {
        showToast(`❌ ${error.message || "Échec du chargement du chauffeur"}`, "error");
      })
  ...
  return (
    <Toast.Provider>
      <div className="p-6">
        <PageBreadcrumb pageTitle="Modifier le chauffeur" />
